...
func main() {
	fmt.Println("=== Cisco FTD Management Adaptive Plugin Test Suite ===\n")

	// Test 1: Basic FTD Detection (Server Certificate Analysis)
	fmt.Println("Test 1: Basic FTD Detection (Server Certificate Analysis)")
	testBasicFTDDetection()

	// Test 2: Enhanced FTD Detection (Certificate Authentication)
	fmt.Println("\nTest 2: Enhanced FTD Detection (Certificate Authentication)")
	testEnhancedFTDDetection()
...
func testBasicFTDDetection() {
	fmt.Println("✅ Testing basic FTD detection without client certificates...")
	...
	testCertificates := []struct {
		...
		{
			subject:    "CN=FTD-VM000000001, OU=Firepower Threat Defense, O=Cisco Systems",
			issuer:     "CN=Cisco Root CA, O=Cisco Systems",
			confidence: 100,
			shouldPass: true,
		},
		{
			subject:    "CN=Firepower-001, OU=Network Security, O=Cisco Systems",
			issuer:     "CN=Cisco Root CA, O=Cisco Systems",
			confidence: 85,
			shouldPass: true,
		},
	...
	for i, test := range testCertificates {
		confidence := analyzeCertificateForFTD(test.subject, test.issuer)
...
func testEnhancedFTDDetection() {
	fmt.Println("✅ Testing enhanced FTD detection with client certificates...")
	...
	scenarios := []struct {
		...
		{
			name:           "Cisco certificate accepted",
			authSuccess:    true,
			protocolAccess: true,
			vulnerable:     true,
		},
...
func testProtocolProbing() {
	...
	probeResponses := []struct {
		...
		{"FTDM_RESPONSE_OK", 35},
		{"FTD_CAPABILITY_RESPONSE", 30},
		{"Firepower threat defense", 25},
...
func analyzeCertificateForFTD(subject, issuer string) int {
	confidence := 0

	subjectUpper := strings.ToUpper(subject)
	issuerUpper := strings.ToUpper(issuer)

	if strings.Contains(subjectUpper, "FTD") {
		confidence += 40
	} else if strings.Contains(subjectUpper, "FIREPOWER") {
		confidence += 35
	} else if strings.Contains(subjectUpper, "THREAT DEFENSE") {
		confidence += 25
	}

	if strings.Contains(subjectUpper, "CISCO") || strings.Contains(issuerUpper, "CISCO") {
		confidence += 25
	}
...
func analyzeProbeResponse(response string) int {
	...
	if strings.Contains(responseUpper, "FTDM") {
		return 35
	} else if strings.Contains(responseUpper, "FTD") {
		return 30
	} else if strings.Contains(responseUpper, "FIREPOWER") {
		return 25
