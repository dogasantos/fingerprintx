...
package checkpoint_log_exporter
...
type LogExporterFingerprint struct {
	...
	LoggingFeatures    []string               `json:"logging_features,omitempty"`
	ExportCapabilities []string               `json:"export_capabilities,omitempty"`
	SecurityInfo       map[string]interface{} `json:"security_info,omitempty"`
}

type LogExporterVendorInfo struct {
...
func (p *Plugin) Run(conn net.Conn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	...
	// 1. Basic Log Exporter detection (no client certificate)
	// 2. Enhanced Log Exporter detection (with client certificate)

	// Tier 1: Basic Log Exporter Detection
	basicResult, err := p.performBasicLogExporterDetection(conn, timeout, target)
	if err != nil {
		return nil, err
	}

	if basicResult == nil {
		return nil, nil // Not Log Exporter
	}

	// If basic detection succeeded, try enhanced detection
	enhancedResult, err := p.performEnhancedLogExporterDetection(conn, timeout, target, basicResult)
	if err != nil {
...
func (p *Plugin) performBasicLogExporterDetection(conn net.Conn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	...
	// Analyze server certificate for Log Exporter patterns
	...
	confidence := p.analyzeCertificateForLogExporter(cert)
...
func (p *Plugin) performEnhancedLogExporterDetection(conn net.Conn, timeout time.Duration, target plugins.Target, basicResult *plugins.Service) (*plugins.Service, error) {
	...
	// Try to communicate using Log Exporter protocol
	logExporterInfo, err := p.performLogExporterProtocolCommunication(tlsConn, timeout)
	if err != nil {
		return nil, err
	}
	...
	fingerprint := LogExporterFingerprint{
		...
		LoggingFeatures:    logExporterInfo.LoggingFeatures,
		ExportCapabilities: logExporterInfo.ExportCapabilities,
...
func (p *Plugin) analyzeCertificateForLogExporter(cert *x509.Certificate) int {
	confidence := 0
...
func (p *Plugin) performProtocolProbing(conn *tls.Conn, timeout time.Duration) int {
	// Send Log Exporter magic bytes
	logExporterProbe := []byte{
		0x4C, 0x4F, 0x47, 0x45, // "LOGE" (Log Exporter)
	...
	if _, err := conn.Write(logExporterProbe); err != nil {
		return 0
	}
	...
	// Analyze response for Log Exporter patterns
	if n >= 4 && string(response[:4]) == "LOGE" {
		return 60
	}
	...
	if strings.Contains(responseStr, "check point") ||
		strings.Contains(responseStr, "log exporter") ||
		strings.Contains(responseStr, "logging") ||
...
type LogExporterInfo struct {
	Product            string                 `json:"product"`
	Version            string                 `json:"version"`
	LoggingFeatures    []string               `json:"logging_features"`
	ExportCapabilities []string               `json:"export_capabilities"`
	SecurityInfo       map[string]interface{} `json:"security_info"`
}

func (p *Plugin) performLogExporterProtocolCommunication(conn *tls.Conn, timeout time.Duration) (*LogExporterInfo, error) {
	// Send authenticated Log Exporter request
	logExporterRequest := []byte{
		0x4C, 0x4F, 0x47, 0x45, // "LOGE" (Log Exporter)
		0x02, 0x00, 0x00, 0x00, // Version 2
		0x00, 0x00, 0x00, 0x20, // Length
		0x00, 0x00, 0x00, 0x02, // Command: GetCapabilities
	...
	if _, err := conn.Write(logExporterRequest); err != nil {
		return nil, err
	}
	...
	// Parse Log Exporter response (simplified)
	info := &LogExporterInfo{
		Product: "Log Exporter Server",
		Version: "Log Exporter vR81.10",
		...
		SecurityInfo: map[string]interface{}{
			...
			"export_protocols": []string{"TLS", "HTTPS", "Secure Syslog"},
		},
	}
...
func (p *Plugin) PortPriority(port uint16) bool {
	return port == 18210 || port == 18211 || port == 18212
}

func (p *Plugin) Name() string {
	return CHECKPOINT_LOG_EXPORTER
}
